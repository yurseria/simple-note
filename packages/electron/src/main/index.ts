...
export function createWindow(): BrowserWindow {
  ...
  win.webContents.on("render-process-gone", (_e, details) => {
    ...
    if (canReload()) {
      ...
      logger.info(
        `renderer crashed (${details.reason}, exitCode:${details.exitCode}) — scheduling reload`,
      );
      setTimeout(() => {
...
app.whenReady().then(() => {
  logger.info(
    `app ready — v${app.getVersion()}, electron ${process.versions.electron}, node ${process.versions.node}`,
  );
  ...
  registerIpcHandlers();
  const storedLang = store.get("language") as UILanguage | undefined;
  const initialLang: UILanguage =
    storedLang ??
    (app.getLocale().toLowerCase().startsWith("ko") ? "ko" : "en");
  buildMenu(initialLang);
